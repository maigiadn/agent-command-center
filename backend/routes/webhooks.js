...
router.post('/', (req, res) => {
    const { name, description, n8nWebhookUrl, status = 'active' } = req.body;
    ...
    try {
      ...
      INSERT INTO webhooks (id, name, description, status, n8nWebhookUrl)
      VALUES (?, ?, ?, ?, ?)
    `);
        stmt.run(id, name, description || '', status, n8nWebhookUrl);
...
router.put('/:id', (req, res) => {
    const { id } = req.params;
    const { name, description, n8nWebhookUrl, status } = req.body;
    ...
    try {
        ...
        const stmt = db.prepare(`
      UPDATE webhooks 
      SET name = ?, description = ?, status = ?, n8nWebhookUrl = ?
      WHERE id = ?
    `);
        stmt.run(name, description || '', status || 'active', n8nWebhookUrl, id);
